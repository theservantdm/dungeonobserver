	...
	while(nextMove == -1) {
		...
		gridArr[targetPos].open > 0) { // is an open area
			if(gridArr[OPC.currentPos].room == "SC") { // OPC is in a secret room
				if(gridArr[targetPos].sDoor > 0) { // target position has the secret door
					nextMove = targetPos;
				}
			}
...
//Math.floor(OPC.abilityScores[_]/2)-5;
...
function CreateMapArray() {
	gridArr = [];
	for(i=0; i<100; i++) {
		gridArr.push({open: 0, room: "", explored: 0, sDoor: 0, encounter: 0});
	}
};
...
function PopulateRandomTunnels() {
	for(i = 0; i < 10; i) { // keep going until you find a number of valid positions
...
function PopulateSecretRoom() {
	for(i=0; i < 2; i) { // keep going until you find a valid position
		secretPosition = Dice(90) + 9; // pick a random location on the grid
		validPosition = 0; // track position validity
		secretDoor = [];
		...
		validPosition > 0) { // is a valid position (beside an open space)
			...
			gridArr[secretPosition].exit = validPosition; // track potential exits
			...
			i ++; // a valid position has been found, stop loop
		}
	}
}
...
function GetExits(tile) {
	...
	gridArr[tile - 1].open > 0) { // west is open
		if(gridArr[tile].room != "SC") { // not in a secret room
			if(gridArr[tile - 1].room != "SC" || // west is not a secret room OR
			gridArr[tile].sDoor > 0) { // tile has secret door
				validExit += 1;
			}
		}
		else if(gridArr[tile].room == "SC" && // in a secret room
		gridArr[tile - 1].sDoor > 0) { // west is secret door
			validExit += 1;
		}
	}
	...
	gridArr[tile + 1].open > 0) { // east is open
		...
		gridArr[tile + 1].sDoor > 0) { // east is secret door
			validExit += 100;
		}
	}
	...
	gridArr[tile - 10].open > 0) { // north is open
		if(gridArr[tile].room != "SC") { // tile is not a secret room
			if(gridArr[tile - 10].room != "SC" || // is not a secret room OR
			gridArr[tile].sDoor > 0) { // tile has secret door
				validExit += 1000;
			}
		}
		else if(gridArr[tile].room == "SC" && // tile is a secret room
		gridArr[tile - 10].sDoor > 0) { // north is secret door
			validExit += 1000;
		}
	}
	...
	gridArr[tile + 10].open > 0) { // south is open
		...
		gridArr[tile + 10].sDoor > 0) { // south is secret door
			validExit += 10;
		}
	}
	return validExit;
